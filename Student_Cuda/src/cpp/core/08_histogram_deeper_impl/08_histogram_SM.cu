...
__host__  void computeHistogram_SM(int* dataImg, int dataLength, int* resultHisto, int lengthHisto);
__global__ static void computeHistogramGPU_SM(int n,int* ptrDevRes, int* ptrDevTabData,int lenght);
__device__ void reduce_intra_thread_histogram_SM(int* ptrDevTabData, int* tabSM, int n);
__device__ static void reduce_inter_block_histogram_SM(int* tabSM, int* tabHistogramGM, int histogramSize);
...
__host__ void computeHistogram_SM(int* dataImg, int dataLength, int* resultHisto, int lengthHisto)
    ...
    cout << endl << "[Histogram Cuda 2 Shared Memory]" << endl;
    ...
    chrono.print("MM In : ");
    chrono.start();
    computeHistogramGPU_SM<<<dg,db,sizeTabSM>>>(dataLength,ptrDevRes, ptrDevTabData,lengthHisto); // asynchrone !!
...
__global__ void computeHistogramGPU_SM(int n,int* ptrDevRes, int* ptrDevTabData,int lenght)
    {
    ...
    //une instance par block
      ...
      reduce_intra_thread_histogram_SM(ptrDevTabData,tabSM, n);
      __syncthreads();
      reduce_inter_block_histogram_SM(tabSM,ptrDevRes, lenght);
  }

__device__ void reduce_inter_block_histogram_SM(int* tabSM, int* tabHistogramGM, int histogramSize){
