...
__host__  void computeHistogram_GM_OP(int* dataImg, int dataLength, int* resultHisto, int lengthHisto);
__global__ static void computeHistogramGPU_GM_OP(int n,int* ptrDevRes, int* ptrDevTabData,int lenght);
__device__ void reduce_intra_thread_histogram_GM_OP(int* ptrDevTabData, int* ptrDevRes, int n);
...
__host__ void computeHistogram_GM_OP(int* dataImg, int dataLength, int* resultHisto, int lengthHisto)
    ...
    cout << endl << "[Histogram Cuda 2 Global Memory OP]" << endl;
    ...
    chrono.print("MM In : ");
    chrono.start();
    computeHistogramGPU_GM_OP<<<dg,db>>>(dataLength,ptrDevRes, ptrDevTabData,lengthHisto); // asynchrone !!
...
__global__ void computeHistogramGPU_GM_OP(int n,int* ptrDevRes, int* ptrDevTabData,int lenght)
    {
    ...
    //une instance par block
      ...
      reduce_intra_thread_histogram_GM_OP(ptrDevTabData,ptrDevRes, n);
      //__syncthreads();
