...
__global__ void colorToGreyMultiGPU(uchar4* ptrDevPixels, int w, int h);

__global__ void affineTransformMultiGPU(uchar4* ptrDevPixels, float a, float b, int w, int h, int offset);
...
__device__ void ecrasementMultiGPU(uchar* tabSM, int halfThread)
    {
...
__device__ void reductionIntraBMultiGPU(uchar* tabSM)
    {
    ...
    while(halfThread>=1)
	{
	ecrasementMultiGPU(tabSM,halfThread);
	__syncthreads();
	halfThread/=2;
	}
    }

__device__ void reductionInterBMultiGPU(uchar* tabSM, uchar* ptrDevResult)
    {
    const int TID_LOCAL = Indice1D::tidLocalBlock();
...
__device__ void reductionIntraTMultiGPU(uchar* tabSM, uchar4* ptrDevPixels,int n)
    {
    const int NB_THREAD = Indice1D::nbThread();
 ...
 */
__global__ void findMinMaxMultiGPU(uchar4* ptrDevPixels, uchar* ptrDevResult,int w, int h)
    {
    ...
    reductionIntraTMultiGPU(tabSM, ptrDevPixels,sizePtrDevPixels);
    reductionIntraBMultiGPU(tabSM);
    reductionInterBMultiGPU(tabSM, ptrDevResult);
    }
