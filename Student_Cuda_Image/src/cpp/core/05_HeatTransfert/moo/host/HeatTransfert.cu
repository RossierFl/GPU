...
HeatTransfert::HeatTransfert(int w, int h, float k):calibreurCuda(0.0,1.0,0.7,0.0)
    ...
    //CalibreurCudas calibreur(0.0,1.0,0.7,0.0);
    //this->calibreurCuda = calibreur;
    ...
    //float **imageHeaterCPU = new float*[h];
    float *imageHeaterCPU = new float[w*h];
...
void HeatTransfert::runGPU(uchar4* ptrDevPixels)
	...
	if((i+1)%2==1)iterationOutput=ptrDevImageB;
	else iterationOutput=ptrDevImageA;
 ...
 * Override
 */
/*DomaineMath* HeatTransfert::getDomaineMathInit(void)
    {
    return NULL;
    }*/
